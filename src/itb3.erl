...
%%     {ok, Wire} = itb3:encrypt_message(Sender, <<"hi">>),
%%     {ok, <<"hi">>} = itb3:decrypt_message(Receiver, Wire),
%%     ok = itb3:free(Receiver),
%%     ok = itb3:free(Sender).
%%
%% Handles are NIF resources: dropping every term reference lets the
%% garbage collector release the Go-side state (libitb3 zeroes key
...
%% module; absent keys are optional fields at their zero value.
...
init(Profile, Opts) ->
    itb3_nif:init_nif(to_bin(Profile), opts_pairs(Opts)).
...
load(Blob, PermMaster, WrapMaster) ->
    itb3_nif:load_nif(Blob, PermMaster, WrapMaster).
...
load_f(Path, PermMaster, WrapMaster) ->
    itb3_nif:load_f_nif(to_bin(Path), PermMaster, WrapMaster).
...
save_f(Pipeline, Path) ->
    itb3_nif:save_f_nif(Pipeline, to_bin(Path)).
...
max_workers(Pipeline, N) ->
    itb3_nif:max_workers_nif(Pipeline, N).
...
rekey(Pipeline, PermMaster, WrapMaster) ->
    itb3_nif:rekey_nif(Pipeline, PermMaster, WrapMaster).
...
encrypt_stream_one_shot(Pipeline, Plain) ->
    itb3_nif:encrypt_stream_one_shot_nif(Pipeline, Plain).
...
decrypt_stream_one_shot(Pipeline, Wire) ->
    itb3_nif:decrypt_stream_one_shot_nif(Pipeline, Wire).
...
stream_read(Stream, MaxBytes) ->
    itb3_nif:stream_read_nif(Stream, MaxBytes).
...
stream_free(Stream) ->
    itb3_nif:stream_free_nif(Stream).
...
inspect(Blob) ->
    json_out(itb3_nif:inspect_nif(Blob)).
...
%% equal to Name. Validation is performed by libitb3; a duplicate name
...
register(Name, Profile) when is_map(Profile) ->
    itb3_nif:register_nif(to_bin(Name), iolist_to_binary(json:encode(Profile)));
register(Name, ProfileJson) ->
    itb3_nif:register_nif(to_bin(Name), to_bin(ProfileJson)).
...
lookup(Name) ->
    json_out(itb3_nif:lookup_nif(to_bin(Name))).
...
set_memory_limit(Bytes) ->
    itb3_nif:set_memory_limit_nif(Bytes).

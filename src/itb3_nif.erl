...
%% load. Argument terms are already normalised by `itb3` (binaries for
...
load() ->
    So = case code:priv_dir(libitb3) of
             {error, bad_name} ->
                 ...
                 filename:join([filename:dirname(Ebin), "priv", "libitb3_nif"]);
             Priv ->
                 filename:join(Priv, "libitb3_nif")
         end,

...
unknown_profile_test() ->
    {error, {unknown_profile, Detail}} = itb3:init(<<"no-such-profile">>, #{}),
...
unknown_opts_key_test() ->
    ...
    ?assertMatch({error, {bad_input, _}},
                 itb3:init(<<"singlemsg-triple-mac-v1">>,
                          #{chunksize => 4096})).
...
malformed_blob_test() ->
    ?assertMatch({error, _}, itb3:load(<<"not a session blob">>)).
...
probe_flip(Receiver, Wire, Pos) ->
    ...
    case itb3:decrypt_message(Receiver, Tampered) of
        {error, {mac_failure, _}} ->
            true;
        {ok, _} ->
...
freed_pipeline_test() ->
    ...
    ok = itb3:free(Pipe),
    ok = itb3:free(Pipe), %% idempotent
    ?assertMatch({error, {bad_handle, _}}, itb3:encrypt_message(Pipe, <<"x">>)),
    ?assertMatch({error, {bad_handle, _}}, itb3:save(Pipe)),
    ?assertMatch({error, {bad_handle, _}}, itb3:encrypt_stream(Pipe)).
...
freed_stream_test_() ->
    {timeout, 120, fun() ->
        ...
        ok = itb3:stream_free(Stream), %% idempotent
        ?assertMatch({error, {bad_handle, _}},
                     itb3:stream_write(Stream, <<"x">>)),
        ?assertMatch({error, {bad_handle, _}}, itb3:stream_end(Stream)),
        ?assertMatch({error, {bad_handle, _}}, itb3:stream_read(Stream, 16)),
        ok = itb3:free(Pipe)
    end}.

badarg_test() ->
    ?assertError(badarg, itb3:encrypt_message(make_ref(), <<"x">>)),
    ?assertError(badarg, itb3:stream_write(make_ref(), <<"x">>)),
    ?assertError(function_clause, itb3:init(<<"p">>, not_opts)).
...
register_test_() ->
    {timeout, 240, fun() ->
        ...
        Profile = #{<<"mode">> => <<"singlemsg-nomac">>,
                    ...
                    <<"parallax">> => false,
                    <<"wrapper">> => false},
        ok = itb3:register(<<"erlang-binding-test-mixed">>, Profile),
        ?assert(lists:member(<<"erlang-binding-test-mixed">>, itb3:profiles())),
        {ok, Record} = itb3:lookup(<<"erlang-binding-test-mixed">>),
        ?assertEqual(Hashes, maps:get(<<"hashes">>, Record)),
        ?assertMatch({error, {profile_exists, _}},
                     itb3:register(<<"erlang-binding-test-mixed">>, Profile)),
        %% Strict record decode on the Go side: an unknown key is
        %% rejected there, not by the binding.
        ?assertMatch({error, {bad_input, _}},
                     itb3:register(<<"erlang-binding-test-badkey">>,

...
round_trip(Profile) ->
    ...
    lists:foreach(
      fun(Plain) ->
              ...
              {ok, Back} = itb3:decrypt_stream_one_shot(Receiver, Wire),
        ...
        lists:foreach(
          fun(Profile) ->
                  ...
                  ?assertMatch({error, {bad_input, _}},
                               itb3:encrypt_stream_one_shot(Sender, <<>>)),
                  ok = itb3:free(Sender)
          end,
...
session_interop_test_() ->
    {timeout, 300, fun() ->
        ...
        {ok, Back} = itb3:decrypt_stream_one_shot(Receiver, Wire2),
        ?assertEqual(Plain, Back),
        ok = itb3:free(Receiver),
        ok = itb3:free(Sender)
    end}.
...
probe_flip(Receiver, Wire, Pos) ->
    ...
    case itb3:decrypt_stream_one_shot(Receiver, Bad) of
        {error, {_, _}} -> true;
